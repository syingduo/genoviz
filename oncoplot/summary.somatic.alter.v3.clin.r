...
input_clin <- my_args[2]
output_file <- my_args[3]

mat <- read.table(input_file, header = T, sep = ",")
clin <- read.table(input_clin, header = T, sep = "\t")
...
colh <- c("ADC" = "red", "Adenocarcinoma" = "orange", "Adenosquamous" = "yellow", "Carcinoid" = "green", "Large cell carcinoma" = "blue", "Squamous" = "purple", "Small cell carcinoma" = "cyan", "LCNEC" = "chocolate4")
colt <- c("metastatic" = "maroon1", "non-metastatic" = "lavender")
...
alter_fun <- list(
  background = function(x, y, w, h) {
    ...
    # grid.rect(x, y, w-unit(0.5, "mm"), h-unit(0.5, "mm"), gp = gpar(fill = "#F2F2F2", col = NA))
  },
...
ht <- oncoPrint(mat,
  ...
  top_annotation = HeatmapAnnotation(Histology = subtype, col = list(Histology = colh, Tumor_type = colt), annotation_legend_param = list(nrow = 1), Tumor_type = Sample_type),
  show_column_names = FALSE,
  # printSamples = T,
  # bottom_annotation = ha,
  # bottom_annotation_height = unit(2, "mm")
  # remove_empty_columns = TRUE,
  heatmap_legend_param = list(title = "Genetic Alterations", nrow = 1)
)

# lgd = Legend(col_fun = colh, title = "Histology", direction = "horizontal")
# %v%
...
## draw plot
pdf(output_file, width = 25, height = 6)
